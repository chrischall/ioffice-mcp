...
- **Every write goes through `confirmWrite` (`src/tools/_confirm.ts`)**: a new mutating tool takes `confirmToken: confirmTokenParam`, appends `CONFIRM_DESCRIPTION` to its description, and calls `confirmWrite(ctx, { tool, action, summary, method, path, body, target, confirmToken })` before `client.request`, passing the exact body it will send (the token is bound to it). Behaviour on clients without elicitation is set by `MCP_CONFIRM_MODE` / `MCP_CONFIRM_TTL_SECONDS` / `MCP_CONFIRM_SECRET` (see README). Add the tool to the `WRITES` table in `tests/confirm.test.ts`.
