...
export async function handleTool(
  ...
  switch (name) {
    case 'io_list_maintenance_requests': {
      const { search, status, spaceId, buildingId, assignedUserId, limit, startAt, orderBy, orderByType } = args as {
        search?: string; status?: string; spaceId?: number; buildingId?: number;
        assignedUserId?: number; limit?: number; startAt?: number; orderBy?: string; orderByType?: string;
      };
    ...
    case 'io_complete_maintenance_request': {
      const { id, resolution } = args as { id: number; resolution?: string };
