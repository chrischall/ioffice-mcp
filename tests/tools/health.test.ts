...
import {
  IOfficeClient as RealIOfficeClient,
  type IOfficeClient,
} from "../../src/client.js";
...
function setup(
  ...
  const call = async () =>
    JSON.parse(
      (await (server as any)._registeredTools.io_healthcheck.handler({}, {}))
        .content[0].text,
    );
...
describe("io_healthcheck", () => {
  ...
  it("never echoes the credential itself", async () => {
    const out = await setup({
      IOFFICE_HOST: "h",
      IOFFICE_TOKEN: "SUPER-SECRET",
    }).call();
    expect(JSON.stringify(out)).not.toContain("SUPER-SECRET");
  });

  it("reports the host, which is half the configuration", async () => {
    expect((await setup(FULL).call()).credential.detail.host).toBe(
      "acme.iofficeconnect.com",
    );
  });
  ...
  it("reports what a real client resolved, without echoing the credential", async () => {
    ...
    (real as unknown as { api: unknown }).api = {
  ...
  it("prefers the token over a username/password pair, as the client does", async () => {
    ...
    expect(new RealIOfficeClient().describeCredential().source).toBe(
      "IOFFICE_TOKEN",
    );
    vi.unstubAllEnvs();
  });
  ...
  it("classifies a non-Error throw without crashing", async () => {
    const out = await setup(FULL, async () => {
      throw "Rate limited by iOffice API";
    }).call();
    expect(out.error.kind).toBe("rate_limited");
  });
});
